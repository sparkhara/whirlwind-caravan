...
class SOM(object):
  def __init__(self, xdim=0, ydim=0, fdim=0, entries=[], norms=[], mqsink=None):
    self.xdim = xdim
    self.ydim = ydim
    self.fdim = fdim
  ...
  def sparseBooleanBestWithSimilarity(self, features, indices):
    """ returns the best similarity result, assuming a sparse boolean vector for the testing example.  features is the maximum number of elements and indices is a unique array of elements that are set to 1 in the sparse example vector """
